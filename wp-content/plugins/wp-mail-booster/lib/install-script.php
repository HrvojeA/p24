...
else
{
	...
	else
	{
		...
		*/
				...
				foreach($mail_booster_table_data as $row)
				{
					switch($row->type)
					{
						...
						case "roles_and_capabilities":
							...
							$roles_capabilities_data_array["contributor_privileges"] = "0,0,0,0,0,0,1,0";
			...
			default:
				if(count($wpdb->get_var("SHOW TABLES LIKE '" . mail_booster() . "'")) != 0 && count($wpdb->get_var("SHOW TABLES LIKE '" . mail_booster_meta() . "'")) != 0)
				{
					...
					if(array_key_exists("roles_and_capabilities", $get_roles_settings_data_array))
					{
						...
						{
							$administrator_privileges_data[7] = $administrator_privileges_data[6];
							$administrator_privileges_data[6] = 1;
						...
						{
							$author_privileges_data[7] = $author_privileges_data[6];
							$author_privileges_data[6] = 0;
							$get_roles_settings_data_array["author_privileges"] = implode(",",$author_privileges_data);
						}

						if(count($editor_privileges_data) == 7)
						{
							$editor_privileges_data[7] = $editor_privileges_data[6];
							$editor_privileges_data[6] = 0;
							$get_roles_settings_data_array["editor_privileges"] = implode(",",$editor_privileges_data);
						}

						if(count($contributor_privileges_data) == 7)
						{
							$contributor_privileges_data[7] = $contributor_privileges_data[6];
							$contributor_privileges_data[6] = 0;
						...
						{
							$subscriber_privileges_data[7] = $subscriber_privileges_data[6];
							$subscriber_privileges_data[6] = 0;
							$get_roles_settings_data_array["subscriber_privileges"] = implode(",",$subscriber_privileges_data);
						}

						if(count($other_privileges_data) == 7)
						{
							$other_privileges_data[7] = $other_privileges_data[6];
							$other_privileges_data[6] = 0;
		...
		update_option("mail-booster-version-number","2.0.2");
	}
}
?>

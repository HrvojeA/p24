...
if(!defined("MAIL_BOOSTER_FILE")) define("MAIL_BOOSTER_FILE",plugin_basename(__FILE__));
if(!defined("MAIL_BOOSTER_DIR_PATH")) define("MAIL_BOOSTER_DIR_PATH",plugin_dir_path(__FILE__));
if(!defined("MAIL_BOOSTER_PLUGIN_DIRNAME")) define("MAIL_BOOSTER_PLUGIN_DIRNAME", plugin_basename(dirname(__FILE__)));
if(!defined("MAIL_BOOSTER_ERROR_LOG_FILE")) define("MAIL_BOOSTER_ERROR_LOG_FILE",MAIL_BOOSTER_DIR_PATH."views/error-logs/error-logs.txt");
@ini_set("log_errors",1);
@ini_set("error_log",MAIL_BOOSTER_ERROR_LOG_FILE);
...
{
	function check_user_roles_mail_booster($user = null)
	{
...
Created By: Tech Banker Team
*/
if(!function_exists("mail_booster_action_links"))
{
	function mail_booster_action_links($plugin_link)
	{
		$plugin_link[] = "<a href=\"http://beta.tech-banker.com/products/mail-booster/\" style=\"color: red; font-weight: bold;\" target=\"_blank\">Go Pro!</a>";
		return $plugin_link;
	}
}
...
if($version == "2.0.2")
{
		...
		if(!function_exists("backend_js_css_for_mail_booster"))
		{
			function backend_js_css_for_mail_booster($hook)
			{
				$pages_mail_booster = array(
					...
					"mail_booster_error_logs",
					"mail_booster_system_information",
					"mail_booster_premium_editions"
				);
				foreach($pages_mail_booster as $page_id => $page)
				{
	...
	*/
	...
	/*
	Function Name: plugin_auto_update_mail_booster
	Parameters: No
	...
	*/
	...
	if(!function_exists("plugin_auto_update_mail_booster"))
	{
		function plugin_auto_update_mail_booster()
		{
			if(!wp_next_scheduled("automatic_updates_mail_booster"))
			{
				wp_schedule_event(time(), "daily", "automatic_updates_mail_booster");
			}
			add_action("automatic_updates_mail_booster", "mail_booster_plugin_autoupdate");
		}
	}
	...
	*/
	if(!function_exists("user_functions_for_mail_booster"))
	{
		function user_functions_for_mail_booster()
		{
			...
			{
				plugin_auto_update_mail_booster();
			}
			else
			{
	...
	*/
	...
	/*
	Function Name: uninstall_script_for_mail_booster
	Parameters: No
	...
	*/
	...
	if(!function_exists("uninstall_script_for_mail_booster"))
	{
		function uninstall_script_for_mail_booster()
		{
			global $wpdb;
			if(is_multisite())
			{
				...
				{
					switch_to_blog($blog_id);
					if(file_exists(MAIL_BOOSTER_DIR_PATH."lib/uninstall-script.php"))
					{
						include MAIL_BOOSTER_DIR_PATH."lib/uninstall-script.php";
					}
					restore_current_blog();
				}
			}
	...
	*/
	...
	Created By: Tech Banker Team
	*/
	register_uninstall_hook( __FILE__, "uninstall_script_for_mail_booster");

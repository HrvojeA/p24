...
function wp_mail_admin_notice() {
    ...
    if ( ( ! get_user_meta($user_id, 'wp_mail_smtp_ignore_notice') ) && 
    	( get_option('wp_smtp_mail_after_week','0') == '1' ) ) {
        echo '<div class="updated"><p>'; 

        printf(__('Awesome, you\'ve been using WP Mailer for more than 1 week. May we ask you to give it a 5-star rating on WordPress? | <a href="%2$s" target="_blank">Ok, you deserved it</a> | <a href="%1$s">I alredy did</a> | <a href="%1$s">No, not good enough</a>'), '?wp_smtp_mailer_nag_ignore=0','https://wordpress.org/plugins/wp-mail-smtp-mailer/');
        echo "</p></div>";
    }
}
...
function wp_mail_smtp_do_this_in_week() {

     update_option( 'wp_smtp_mail_after_week', '1' );
}
add_action( 'wp_mail_smtp_new_event','wp_mail_smtp_do_this_in_week' );

wp_schedule_single_event( time() + 604800, 'wp_mail_smtp_new_event' );
